...
calculate_test([H|T]) ->
    case T of 
        [] ->
            hd(H);
        _ ->
            Pairs = lists:map(fun(E) -> make_pairs(E, []) end, T),
             hd(H) + choose_max(hd(hd(Pairs)), tl(Pairs))
    end.

make_pairs([A, B | T], Acc) ->
    make_pairs([B | T], [{A,B} | Acc]);
make_pairs([_], Acc) ->
    lists:reverse(Acc).

choose_max({A, B}, [[PA, PB | _]]) ->
    % ed({{A, B}, PA, PB}),
    max(
        A + choose_max(PA, []),
        B + choose_max(PB, [])
    );
choose_max({A, B}, [[PA, PB | _] | Lines]) ->
    % ed({{A, B},PA, PB, Lines}),
    max(
        A + choose_max(PA, Lines),
        B + choose_max(PB, [tl(Line) || Line <- Lines])
    );
choose_max({A, B}, []) ->
    % ed({A, B}),
    max(
