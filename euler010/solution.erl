...
calculate(Ts) ->
    % 78499th prime = 1000003,
    P78499 = 1000003,
    P1_P78499 = n_prime(lists:seq(2, P78499), ceiling(math:sqrt(P78499)), []),
    % io:format("~p\n", [P1_P78499]),
    lists:map(fun(T) -> calculate_test(T, P1_P78499, 0) end, Ts).

calculate_test(N, [H | T], Sum) when H =< N ->
    calculate_test(N, T, Sum + H);
calculate_test(N, [H|_], Sum) when H > N ->
    Sum.
